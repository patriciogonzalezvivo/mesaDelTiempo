...
public:
	...
	LineDot(int _idN, const ofxRing *_toRing, ofVec2f _loc, bool _fromCenter){
		idN = _idN;
		lnk = NULL;
		setSpace(toRing);
		setColor(_loc);
	...
	LineDot& setSpace(const ofxSpace *toSpace)	{ space = toSpace; return * this;};
	LineDot& setLnk(const ofxParticle *toLnk)	{ lnk = toLnk; return * this;};
